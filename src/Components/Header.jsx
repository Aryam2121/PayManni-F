  ...
  return (
    ...
    >
      ...
      >
        <img src={Paymanniicon} alt="PayManni Logo" className="h-10" />
        <motion.span
  className="text-3xl font-bold tracking-wide text-white font-[cursive] italic drop-shadow-lg"
...
>
  PayManni
      ...
      {/* Search Bar */}
      <motion.div
        className={`hidden md:flex items-center px-4 py-2 rounded-full w-1/3 ${
          darkMode ? "bg-gray-800 text-white" : "bg-gray-100 text-gray-900"
        } shadow-inner`}
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      <nav className="hidden md:flex space-x-6">
        {["Home", "Wallet", "Recharge", "Transactions"].map((item) => (
          <motion.a
            key={item}
            href={`/${item.toLowerCase()}`}
            className="text-base font-medium hover:text-indigo-500 transition duration-300"
            whileHover={{ scale: 1.1 }}
          >
            {item}
          </motion.a>
      ...
      {/* Mobile Menu Toggle */}
      <button onClick={toggleMenu} className="md:hidden text-2xl focus:outline-none ml-3">
        <motion.div animate={{ rotate: isMenuOpen ? 90 : 0 }} transition={{ duration: 0.5 }}>
          {isMenuOpen ? <FaTimes /> : <FaBars />}
        </motion.div>
      </button>
